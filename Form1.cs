...
namespace MixMaster_Packet_Descryptor
{
    public partial class Form1 : Form
    {
        ...
        public static byte bin_key = 0xbc;

        public static byte GetPubKey(byte pub)
        {
            return InverseKey[int.Parse(pub.ToString("X"), System.Globalization.NumberStyles.HexNumber)];
        }

        public static byte[] Decrypt(byte[] data, byte binKey)
        {
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                Decrypted[i] = (byte)(xor2 ^ binKey);
            }
            return Decrypted;

        }

        public static byte[] Encrypt(byte[] data, byte binKey)
        {
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                byte xor1 = (byte)(bin_key ^ key[keyValue]);
                byte xor2 = (byte)(xor1 ^ data[i]);
                Encrypted[i] = (byte)(xor2 ^ binKey);
            }
            return Encrypted;
        }
        public static byte Decrypt(byte data, byte binkey, int keyValue)
        {
            byte Encrypted;

            byte xor1 = (byte)(bin_key ^ key[keyValue]);
            byte xor2 = (byte)(xor1 ^ data);
            Encrypted = (byte)(xor2 ^ binkey);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string[] InputSplited = input.Split((char)0x20);

            foreach(var hexValue in InputSplited)
            {
                novo += hexValue;
            }
            novo = HexStringToString(novo);
        ...
        string HexStringToString(string HexString)
        {
            ...
            int value = int.Parse(textBox3.Text, System.Globalization.NumberStyles.HexNumber);
            byte MyKey = (byte)value;
            ...
            for (int i = 0; i < HexString.Length / 2; i++)
            {
                ...
                byte newhexbyte = Decrypt(hexByte, GetPubKey(MyKey), keyValue);

                textBox2.Text += newhexbyte.ToString("X2");
            }
            return stringValue;
        }
